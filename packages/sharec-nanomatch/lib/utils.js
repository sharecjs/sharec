...
utils.instantiate = function (ast, options) {
  ...
  utils.define(snapdragon, 'parse', function (str, options) {
    const parsed = Snapdragon.prototype.parse.call(this, str, options)
    parsed.input = str
...
utils.combineDupes = function (input, patterns) {
  ...
  const substr = patterns.join('|')
  const regex = new RegExp('(' + substr + ')(?=\\1)', 'g')
...
utils.equalsPattern = function (pattern, options) {
  ...
  return function fn(filepath) {
    const equal = utils._equals(filepath, unixify(filepath), pattern)
...
utils.containsPattern = function (pattern, options) {
  ...
  return function (filepath) {
    const contains = utils._contains(filepath, unixify(filepath), pattern)
